...
IF %1.==--help. (
    ...
    ECHO    --without-peer_selftest  do not build peer_selftest.exe
    GOTO :eof
)
...
ECHO. >> platform.h
ECHO #define PEER_HAVE_WINDOWS 1>> platform.h
...
IF EXIST "..\..\..\libzmq" (
    ...
    ECHO Peer cannot build without libzmq
...
IF EXIST "..\..\..\czmq" (
    ...
    ECHO Peer cannot build without czmq
...
IF EXIST "..\..\..\zyre" (
    ...
    ECHO Peer cannot build without zyre
...
:with_draft
    ...
    ECHO #define PEER_BUILD_DRAFT_API 1>>platform.h
    GOTO end_draft
:no_draft
    ECHO Building without draft API (stable + legacy API)
    ECHO #undef PEER_BUILD_DRAFT_API 1>>platform.h
:end_draft
ECHO. >> platform.h
