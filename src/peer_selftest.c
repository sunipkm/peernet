...
{
    test_item_t *item;
    printf ("Running peer selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef PEER_BUILD_DRAFT_API // selftest is still in draft
        else
            peer_private_selftest (verbose, item->subtest);
#endif // PEER_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    if (test) {
        printf ("Running peer test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef PEER_BUILD_DRAFT_API // selftest is still in draft
        else
            peer_private_selftest (verbose, test->subtest);
#endif // PEER_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
